<script>
    ...
    async function handleReject(paper_id) {
      const response = await fetch("http://localhost:3000/request/delete_request", {
      method: "POST",
      headers: {
    ...
    <main>
      ...
      <div class="cards" >
        ...
        <div class = "border_style">
          ...
          <div class="two-column" style="display: block;margin-top:2%" button-container >
            <button on:click={handleReject(item.paper_id)} style="background-color:red;">Reject</button>
            <button on:click={handleAccept(item.paper_id)} style="background-color:green;">Accept</button>
          </div>
