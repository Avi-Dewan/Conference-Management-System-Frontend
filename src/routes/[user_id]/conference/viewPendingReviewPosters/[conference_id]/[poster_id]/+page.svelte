<script>
    ...
    let url = `${import.meta.env.VITE_BACKEND}/conference/${conference_id}/viewPendingReviewPosters/${poster_id}`;
        ...
        {#each posters as item}
          ...
          <div style="margin-top: 70px;">
            ...
            <button on:click={() => window.open(item.pdf_link, "_blank")}
              >View file</button
            >
            <a href="/{user_id}/conference/assignReviewer/{item.poster_id}">
              Assign
            </a>
          </div>
        {/each}
